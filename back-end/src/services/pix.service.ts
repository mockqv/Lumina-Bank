...
export const createPixKey = async (user_id: string, data: CreatePixKeyData): Promise<PixKey> => {
  ...
  if (key_type === 'cpf' || key_type === 'cnpj') {
    const userResult = await pool.query('SELECT cpf FROM users WHERE id = $1', [user_id]);
    if (userResult.rows.length === 0) {
        throw new Error("User not found.");
    }
    const encryptedUserCpf = userResult.rows[0].cpf;
    const decryptedUserCpf = decrypt(encryptedUserCpf);

    if (decryptedUserCpf !== final_key_value) {
        throw new Error(`The provided ${key_type.toUpperCase()} does not match the account holder's document.`);
    }
  }
